...
document.writeln('<iframe id=endDateLayer frameborder=0 width=165 height=220 style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #8f939f 1px solid;BORDER-TOP: #8f939f 1px solid;BORDER-LEFT: #8f939f 1px solid;';
strFrame+='BORDER-BOTTOM: #8f939f 1px solid;BACKGROUND-COLOR: #8f939f;font-family:宋体;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#8f939f bgcolor=#8f939f >';
...
strFrame+='                <td width=16 align=center bgcolor=#8f939f style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='        onclick="parent.meizzPrevM()" title="'+str_forward_month+'" ><b >&lt;</b></td>';
strFrame+='       <td width=60 align="center" bgcolor="#8f939f"  style="font-size:12px;cursor:hand" ';
strFrame+='           onmouseover="style.backgroundColor=\'#aaccf3\'"';
strFrame+='        onmouseout="style.backgroundColor=\'#8f939f\'" ';
strFrame+='        onclick="parent.tmpSelectYearInnerHTML(this.innerText.substring(0,4))" ';
strFrame+='        title="'+str_selhere_year+'"><span  id=meizzYearHead></span></td>';
strFrame+='       <td width=48 align="center" style="font-size:12px;font-color: #ffffff;cursor:hand" ';
strFrame+='        bgcolor="#8f939f" onmouseover="style.backgroundColor=\'#aaccf3\'" ';
strFrame+='        onmouseout="style.backgroundColor=\'#8f939f\'" ';
...
strFrame+='        title="'+str_selhere_month+'"><span id=meizzMonthHead ></span></td>';
strFrame+='       <td width=16 bgcolor=#8f939f align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='        onclick="parent.meizzNextM()" title="'+str_back_month+'" ><b >&gt;</b></td>';
strFrame+='      </tr>';
strFrame+='     </table></td></tr>';
strFrame+='    <tr><td width=142 height=18 >';
strFrame+='     <table border=0 cellspacing=0 cellpadding=2 bgcolor=#8f939f ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+='    BORDERCOLORLIGHT=#8f939f BORDERCOLORDARK=#FFFFFF width=140 height=20  style="cursor:' + (bMoveable ? 'move':'default') + '">';
strFrame+='    <tr><td style="font-size:12px;color:#ffffff" width=20>Sun</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" >Mon</td><td style="font-size:12px;color:#FFFFFF">Tue</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" >Wed</td><td style="font-size:12px;color:#FFFFFF" >Thu</td>';
strFrame+='<td style="font-size:12px;color:#FFFFFF" >Fri</td><td style="font-size:12px;color:#FFFFFF" >Sat</td></tr>';
strFrame+='</table></td></tr>';
strFrame+='  <tr ><td width=142 height=120 >';
strFrame += '    <table border=1 cellspacing=2 cellpadding=2 BORDERCOLORLIGHT=#8f939f BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 >';
...
strFrame += '       onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="' + str_clear_datetime + '">C</span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="'+str_now_datetime+'">N</span>&nbsp;&nbsp;<span style="cursor:hand" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="'+str_close_datetime+'">X&nbsp;</span></td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame+='          <tr bgcolor="#8f939f"><td id=bUseTimeLayer width=30  style="cursor:hand" title="'+str_isopen_time+'"';
strFrame+='    onmouseover="style.backgroundColor=\'#aaccf3\'" align=center onmouseout="style.backgroundColor=\'#8f939f\'"';
...
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#8f939f\'"';
strFrame+=' title="'+str_clickhere_time+'" align=center width=42>' ;
...
function setday(tt,obj) //主调函数
{
 ...
 if (arguments.length > 2){alert(str_inputvars_err1);return;}
 if (arguments.length == 0){alert(str_inputvars_err2);return;}
 ...
 {
  bUseTime=true;   //如果是FALSE首先就是时分秒不显示
  bImgSwitch();
  odatelayer.bUseTimeLayer.innerHTML=bImg;
  meizzWriteHead(meizzTheYear,meizzTheMonth);
 }
...
{
 if (strYear.match(/\D/)!=null){alert(str_inputyear_err1);return;}
 var m = (strYear) ? strYear : new Date().getFullYear();
 if (m < 1000 || m > 9999) {alert(str_inputyear_err2);return;}
 var n = m - 50;
 if (n < 1000) n = 1000;
...
{
 if (strMonth.match(/\D/)!=null){alert(str_inputmonth_err1);return;}
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
 ...
 if (strHour.match(/\D/)!=null){alert(str_inputhour_err1);return;}
...
function tmpSelectMinuteInnerHTML(strMinute) //分钟的下拉框
{
 ...
 if (strMinute.match(/\D/)!=null){alert(str_inputminute_err1);return;}
...

{
 ...
 if (strSecond.match(/\D/)!=null){alert(str_inputsecond_err1);return;}
...
function meizzSetDay(yy,mm) //主要的写程序**********
{
 ...
 //for (i=0;i<firstday;i++)meizzWDay[i]=GetMonthCount(mm==1?yy-1:yy,mm==1?12:mm-1)-firstday+i+1 //上个月的最后几天-jwb20140313
 for (i = 0; i < firstday; i++) { meizzWDay[i] = "";}

 for (i = firstday; day1 < GetMonthCount(yy, mm) + 1; i++) { meizzWDay[i] = day1; day1++; }     //当月所有天

 //for (i = firstday + GetMonthCount(yy, mm); i < 37; i++) { meizzWDay[i] = day2; day2++; }     //下个月的前几天-jwb20140313
 for (i = firstday + GetMonthCount(yy, mm); i < 37; i++) { meizzWDay[i] = ""; day2++; } 
 
 for (i = 0; i < 37; i++)
 {
  var da = eval("odatelayer.meizzDay"+i) //书写新的一个月的日期星期排列
  
  //初始title信息不显示, onclick禁用-jwb20140313
  da.title = "";
  da.onclick = "";
 ...
 if (meizzWDay[i]!="")
 {
  ...
  da.borderColorLight="#8f939f";
  da.borderColorDark="#8f939f";
  da.style.color="#1478eb";
  if(i<firstday)  //上个月的部分
  {
   da.innerHTML="<b><font color=#BCBABC>" + meizzWDay[i] + "</font></b>";
   //da.title=(mm==1?12:mm-1) +"-" + meizzWDay[i] + " ";      //上个月的title信息不用显示-jwb20140313
   //da.onclick=Function("meizzDayClick(this.innerText,-1)"); //上个月的onclick禁用-jwb20140313
   ...
   else
   {
    ...
    meizzWDay[i] == new Date().getDate()) ? "#8f939f" : "#f5f5f5");
    //将选中的日期显示为凹下去
  ...
  {
   da.innerHTML="<b><font color=#BCBABC>" + meizzWDay[i] + "</font></b>";
   //da.title=(mm==12?1:mm+1) +"-" + meizzWDay[i] + " ";     //下个月的title信息不用显示-jwb20140313
   //da.onclick=Function("meizzDayClick(this.innerText,1)"); //下个月的onclick禁用-jwb20140313
...
//////////////////////////////////
/*
function Pause(obj, iMinSecond, n, ex) 
{
    ...
    setTimeout("GoOn(" + ind + "," + n + "," + ex +")", iMinSecond);
}
function GoOn(ind, n, ex) 
{
    var obj = window.eventList[ind];
    window.eventList[ind] = null;
    if (obj.NextStep) obj.NextStep(n, ex);
    else obj();
}
...
function meizzDayClick(n,ex) //点击显示框选取日期，主输入函数*************
{
 ...
 else {closeLayer(); alert(str_inputctrl_err);}
 closeLayer();
