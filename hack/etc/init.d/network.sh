...
if [ -e /mnt/sd/wpa-hisilicon.conf ]
then
  echo "[network.sh] wpa.conf found on sd card, copying it over ..."
  cp /mnt/sd/wpa-hisilicon.conf /mnt/mtd/wpa-hisilicon.conf
fi

# Check if an SSID is configured  
ssid=`cat /mnt/mtd/wpa-hisilicon.conf | grep ssid | cut -d '"' -f2`
	...
	# Cannot connect to SSID, check if the ssid is down or else passkey is wrong, check if there is a backup WiFi and try that one 
		...
		# SSID is online, could not connect to it must be credential issue check for backup ssid
			...
			then
				...
				mv /mnt/mtd/wpa.conf2 /mnt/mtd/wpa-hisilicon.conf
				reboot
			else
